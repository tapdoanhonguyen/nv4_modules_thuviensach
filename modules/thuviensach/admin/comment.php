...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM vidoco_thuviensach_comment WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE vidoco_thuviensach_comment SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM vidoco_thuviensach_comment  WHERE id = ' . $db->quote($id));
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Comment', 'ID: ' . $id, $admin_info['userid']);
...
if($sach_id>0){
    $where .= ' AND sach_id='.$sach_id;
    $base_url .= '&sach_id=' . $sach_id;
}
if($rate>0){
    $where .= ' AND rate='.$rate;
    $base_url .= '&rate=' . $rate;
...
if($userid>0){
    $xtpl->assign('username', get_info_user($userid)['last_name'].' '.get_info_user($userid)['first_name']);
}else{
    $xtpl->assign('username', 'Tất cả người đánh giá');
}
$xtpl->assign('sach_id', $sach_id);
if($sach_id>0){
    $xtpl->assign('name_sach', get_info_sach($sach_id)['name_sach']);
}else{
    $xtpl->assign('name_sach', 'Tất cả sách');
}
for($i=0;$i<=5;$i++){
    if($i==$rate){
    ...
    $xtpl->assign('key', $i);
    if($i==0){
        $xtpl->assign('title', 'Tất cả số sao');
    }else{
        $xtpl->assign('title', $i.' sao');
    }
...
while ($view = $sth->fetch()) {
    ...
    $view['sach_id']=get_info_sach($view['sach_id'])['name_sach'];
