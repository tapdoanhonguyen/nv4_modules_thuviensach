...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    if (isset($row['status']))     {
        ...
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_muonsach SET status=' . intval($status) . ', time_edit='.NV_CURRENTTIME.', user_edit='.$admin_info['userid'].' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        ...
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Muonsach', 'ID: ' . $id, $admin_info['userid']);
...
$xtpl = new XTemplate('muonsach.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
