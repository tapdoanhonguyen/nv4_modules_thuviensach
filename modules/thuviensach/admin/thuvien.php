...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_thuvien  WHERE id = ' . $db->quote($id));
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Thuvien', 'ID: ' . $id, $admin_info['userid']);
...
$districtid = $nv_Request->get_title( 'districtid', 'post,get' ,0);
$userid_quanly = $nv_Request->get_title( 'userid_quanly', 'post,get' ,0);
...
if ( !empty( $q ) ) {
    $where .= ' AND (name_thuvien LIKE "%" "'.$q. '" "%" OR ma_thuvien LIKE "%" "'.$q. '" "%")';
    $base_url .= '&q=' . $q;
}
...
if($userid_quanly>0){
    $where .=' AND userid_quanly='.$userid_quanly;
    $base_url .='&userid_quanly='.$userid_quanly;
}
...
$db->sqlreset()
->select('COUNT(*)')
->from('' . $db_config['prefix'] . '_' . $module_data . '_thuvien')
->where('1=1'.$where);
...
$xtpl = new XTemplate('thuvien.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
$xtpl->assign('userid_quanly', $userid_quanly);
$xtpl->assign('thuvien_add', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=thuvien_add');
if ( $ngay_tu > 0 )
...
if($userid_quanly>0){
    $xtpl->assign('userid_quanly_name', get_info_user($userid_quanly)['first_name'].' '.get_info_user($userid_quanly)['last_name']);
}else{
    $xtpl->assign('userid_quanly_name', 'Tất cả');
}

if($provinceid>0){
    $xtpl->assign('provinceid_name', get_info_province($provinceid)['title']);
}else{
    $xtpl->assign('provinceid_name', 'Tất cả tỉnh thành');
}

if($districtid>0){
    $xtpl->assign('districtid_name', get_info_district($districtid)['title']);
}else{
    $xtpl->assign('districtid_name', 'Tất cả quận huyện');
}
...
while ($view = $sth->fetch()) {
    $view['number'] = $number++;
    $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=thuvien_add&amp;id=' . $view['id'];
    ...
    $view['provinceid']=get_info_province($view['provinceid'])['title'];
    $view['districtid']=get_info_district($view['districtid'])['title'];
    $view['userid_quanly']=get_info_user($view['userid_quanly'])['last_name'].' '.get_info_user($view['userid_quanly'])['first_name'];
...
$page_title = $lang_module['thuvien'];
