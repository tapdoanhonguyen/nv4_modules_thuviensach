...
if (!nv_function_exists('nv_chuyenmuc_info_new')) {
    /**
     ...
     * @return string
     */
    function nv_block_config_thuviensach_chuyenmuc_new($module, $data_block, $lang_block)
    {
     ...
     * @return array
     */
    function nv_block_config_thuviensach_chuyenmuc_new_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string
     */
    function nv_chuyenmuc_info_new($block_config)
    {
        ...
        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file . '/block.chuyenmuc_new.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $module_file . '/block.chuyenmuc_new.tpl')) {
        ...
        $sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_sach WHERE danhmuc_id IN(' . $catid  . ') OR danhmuc_id IN(SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_cat WHERE parentid IN ( ' . $catid . '))  ORDER BY id LIMIT 20';


        $array_sach = $nv_Cache->db($sql, 'id', $module);

        $xtpl = new XTemplate('block.chuyenmuc_new.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $module_file);
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        $content = nv_chuyenmuc_info_new($block_config);
    }
}
