...
if (!nv_function_exists('nv_chuyenmuc_info_new_2')) {
    /**
     ...
     * @return string
     */
    function nv_block_config_thuviensach_chuyenmuc_new_2($module, $data_block, $lang_block)
    {
     ...
     * @return array
     */
    function nv_block_config_thuviensach_chuyenmuc_new_2_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string
     */
    function nv_chuyenmuc_info_new_2($block_config)
    {
        ...
        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file . '/block.chuyenmuc_new_2.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $module_file . '/block.chuyenmuc_new_2.tpl')) {
        ...
        $sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_sach WHERE (danhmuc_id IN(' . $catid  . ') OR danhmuc_id IN(SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_cat WHERE parentid IN ( ' . $catid . '))) AND id IN(SELECT sach_id FROM ' . $db_config['prefix'] . '_' . $module_data . '_muonsach_sach WHERE muonsach_id IN (SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_muonsach WHERE status = 1 OR status = 2)) ORDER BY number_view DESC LIMIT 10';

        
        $array_sach = $nv_Cache->db($sql, 'id', $module);

        $xtpl = new XTemplate('block.chuyenmuc_new_2.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $module_file);
        ...
        if (!empty($array_sach)) {
            foreach ($array_sach as $key => $value) {
                ...
                $value['thong_tin_danh_gia'] = round($db->query('SELECT avg(rate) FROM '.$db_config['prefix'].'_'.$module_data.'_comment where sach_id='.$value['id'])->fetchColumn(),1);
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        $content = nv_chuyenmuc_info_new_2($block_config);
    }
}
