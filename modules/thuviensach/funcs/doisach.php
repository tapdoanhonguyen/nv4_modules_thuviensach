...
if($mod=="dong_y"){
    ...
    if($status == 1){
        $db->query('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_doi_sach SET status = ' . $status);
    }else{
        $db->query('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_doi_sach SET status = ' . $status);
        $info_doi_sach = $db->query('SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_doi_sach WHERE id = ' . $id)->fetch();
        $so_luong = $db->query('SELECT number FROM ' . $db_config['prefix'] . '_' . $module_data . '_soluongsach WHERE sach_id = ' . $info_doi_sach['id_sach'] . ' AND thuvien_id = ' . $info_doi_sach['id_thu_vien'])->fetchColumn();
        $so_luong = $so_luong + 1;
        $db->query('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_soluongsach SET number = ' . $so_luong);
    }
    

    $json[] = ['status'=>'OK', 'text'=>'Thành công!'];
...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . $db_config['prefix'] . '_' . $module_data . '_doi_sach WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_doi_sach SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_doi_sach  WHERE id = ' . $db->quote($id));
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Doisach', 'ID: ' . $id, $admin_info['userid']);
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    ->where('id_thu_vien='.$thuvien_id . $where);
...
$xtpl = new XTemplate('doisach.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['info_sach']['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_name. '/sach/' . $view['info_sach']['image'];
        $view['info_sach']['link'] = NV_BASE_SITEURL . $module_name . '/' . $view['info_sach']['alias'];
        if($view['status'] == 1){
            $xtpl->assign('VIEW', $view);
            $xtpl->parse('main.view.loop.dong_y');
        }else if($view['status'] == 2){
            $xtpl->assign('VIEW', $view);
            $xtpl->parse('main.view.loop.khong_dong_y');
        }else{
            $xtpl->assign('VIEW', $view);
            $xtpl->parse('main.view.loop.chua_dong_y');
        }
...
$page_title = $lang_module['doisach'];
